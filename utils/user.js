  ...
  try {
    ...
    for (const groupRef of groups) {
      const logsQuery = query(
        collection(db, "logs"),
        where("groups", "==", groupRef) // Match logs belonging to the group
      );
    ...
    console.log(pendingVotes.length > 0 ? pendingVotes : false);
    // Return results
...
export const updateUserVote = async (logRef, userRef, voteType) => {
  try {
    if (!["approve", "deny", "unsure"].includes(voteType)) {
      throw new Error(
        'Invalid vote type. Must be "approve", "deny", or "unsure".'
      );
    }

    const voteField = `vote${
      voteType.charAt(0).toUpperCase() + voteType.slice(1)
    }`; // Dynamically determine field
